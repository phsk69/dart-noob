...
int getMaxHappiness(List<String> names, List<List<Object>> relationships,
  ...
  if (names.isEmpty) {
    return calculateHappinessForArrangement(currentArrangement, relationships);
  }
...
class Day13P1Solver extends AoCSolver {
  ...
  Either<String, String> solve() {
    try {
      ...
      return Right('Day13P1Solver: $maxHappiness');
    } catch (e) {
...
class Day13P2Solver extends AoCSolver {
  ...
  Either<String, String> solve() {
    try {
      ...
      var allNames = guests.map((g) => g[0] as String).toSet().toList();
      var yourName = "BingBong";
      allNames.add(yourName);

      for (var name in allNames.where((n) => n != yourName)) {
        guests.add([yourName, 0, name]);
        guests.add([name, 0, yourName]);
      }
      var maxHappiness = getMaxHappiness(allNames, guests, []);
