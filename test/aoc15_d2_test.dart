...
void main() {
  group('aoc15_d2_p1', () {
    ...
    test('checks aoc 15 d2 p1 copilot for correctness', () async {
      var result = await solveAoc15D2P1CoPilot(inputPath);
      expect(result, expected);
    });

    test('checks aoc 15 d2 p1 listoflists for correctness', () async {
      var result = await solveAoc15D2P1ListOfLists(inputPath);
      expect(result, expected);
    });

    test('checks aoc 15 d2 p1 functional listoflists for correctness',
        () async {
      var result = await solveAoc15D2P1FunctionalListOfLists(inputPath);
      expect(result, expected);
    });

    test('checks aoc 15 d2 p1 functional strings for correctness', () async {
      var result = await solveAoc15D2P1FunctionalFromStrings(inputPath);
      expect(result, expected);
    });

    test('checks aoc 15 d2 p1 parallel for correctness', () async {
  ...
  group('aoc15_d2_p2', () {
    ...
    test('checks aoc 15 d2 p2 ListOfLists for correctness', () async {
